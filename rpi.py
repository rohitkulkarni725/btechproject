...
while True:
    ...
    tags = soup('p')
    m_field = int((tags[0]).split(':')[2][1])
    t_field = int(str(tags[0]).split(':')[3][1])
    h_field = int(str(tags[0]).split(':')[4][1])
    li_field = int(str(tags[0]).split(':')[5][1])

    field_no = [m_field,t_field,h_field,li_field]
